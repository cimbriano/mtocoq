...
% TODO : Mention the questions "will this constrain extending the type system to allow each element of an array to occupy its own OramBank"?
...
In hind sight, we could of replace these two a single induction on the length of the trace, instead of worrying about the number of statements in the program.
After we condier each single line program, we do structural induction on program concatenation.

In proving the theorem, it took some effort to convice Coq of things that seemed basic.  For example, convincing it that changing a high security variable maintains low memory equvalence, or
